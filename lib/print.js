...
export default async function (m, conn = { user: {} }) {
    // 1. Evitar logs de estados (historias) para no saturar
    ...
    // 2. Manejo seguro de imágenes en consola
    let img
    try {
        if (global.opts['img']) {
        ...
        // Ignoramos errores de imagen silenciosamente
    }

    // 3. Cálculo de tamaño seguro (sin errores si no hay msg)
    let filesize = 0
    try {
        filesize = (m.msg ?
    ...
    // 4. Limpieza del Tipo de Mensaje (Evita WAMessageStubType feos)
    let messageType = m.mtype ? m.mtype.replace(/message$/i, '').replace('audio', m.msg.ptt ? 'PTT' : 'audio').replace(/^./, v => v.toUpperCase()) : 'Sistema'
    if (m.messageStubType) messageType = 'Notif. Grupo' // Simplifica avisos de sistema

    // 5. Preparación de Info de Usuario (Evita undefined)
    let userInfo = user ? ` | ${user.exp} EXP | ${user.limit || '?'} L | NvL ${user.level}` : ' | Usuario Nuevo'

    // Separador Navideño
    console.log(chalk.green.dim('~'.repeat(20)) + chalk.red.bold(' 🎄 M E R R Y  C H R I S T M A S 🎄 ') + chalk.green.dim('~'.repeat(20)))
...
`.trim())
    ...
    // 6. Manejo del Texto (Comandos y Mensajes)
