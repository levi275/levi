...
*Optimizado para rendimiento y corrección de fugas de memoria*
*/
...
let handler = async (m, { conn, args, usedPrefix, command, isOwner }) => {
    ...
    // Optimización: Filtrado más eficiente de conexiones activas
...
export async function RubyJadiBot(options) {
    ...
    // Ejecución principal envuelta en exec (Mantenido por compatibilidad)
    exec(comb.toString("utf-8"), async (err, stdout, stderr) => {
        ...
        const connectionOptions = {
            logger: pino({ level: "silent" }), // Cambiado a silent para mejorar rendimiento
        ...
        // Variable para controlar el intervalo de limpieza y evitar fugas de memoria
        ...
        async function connectionUpdate(update) {
            ...
            if (qr && !mcode) {
                if (m?.chat) {
                    // Optimización: Scale reducido a 5 para generación más rápida
                    txtQR = await conn.sendMessage(m.chat, { image: await qrcode.toBuffer(qr, { scale: 5 }), caption: rtx.trim()}, { quoted: m})
                } else {
                    return 
                }
                if (txtQR && txtQR.key) {
            ...
            const endSesion = async (loaded) => {
                if (!loaded) {
                    ...
                    // LIMPIEZA CRITICA: Borrar intervalo
            ...
            if (connection === 'close') {
                // Asegurar limpieza de intervalo al cerrar
                if (cleanUpInterval) clearInterval(cleanUpInterval)
                ...
                if (reason === 440) {
                    ...
                    endSesion(false) // Asegurar limpieza
                }
        ...
        // LIMPIEZA DE INTERVALOS ZOMBIES
        // Asignamos el intervalo a una variable para poder borrarlo después
        cleanUpInterval = setInterval(async () => {
            if (!sock.user || sock.ws.readyState === ws.CLOSED) {
                ...
                clearInterval(cleanUpInterval) // Detener este intervalo
            }
        }, 60000)
        ...
        let creloadHandler = async function (restatConn) {
            ...
            if (restatConn) {
                if (cleanUpInterval) clearInterval(cleanUpInterval) // Limpieza preventiva
