...
handler.all = async function (m) {
    ...
    if (/^A Bueno master|Bueno master|Bueno Máster|🫂$/i.test(m.text) && chat.audios) {
        ...
        this.sendMessage(m.chat, { audio: { url: vn }, ...vnOptions }, { quoted: m })
    }
    ...
    if (chat.audios && m.text.match(/(Calla Fan de bts|bts|Amo a bts)/gi)) {
        ...
        if (media === 'audio') await this.sendMessage(m.chat, { audio: { url: vn }, ...vnOptions }, { quoted: m });
    ...
    if (chat.audios && m.text.match(/(Feliz navidad|feliz navidad|Merry Christmas|merry chritmas)/gi)) {
        ...
        this.sendPresenceUpdate('recording', m.chat)
        this.sendMessage(m.chat, { audio: { url: vn }, mimetype: 'audio/mp4', ptt: true, fileName: 'voice.m4a' }, { quoted: m })
    }
