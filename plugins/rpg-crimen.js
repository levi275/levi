...
const handler = async (m, { usedPrefix }) => {
  ...
  if (roll < jailChance + successChance) {
    const amount = Math.floor((Math.random() * 28000 + 14000) * job.crimeRewardMultiplier * (user.premium ? 1.18 : 1));
  ...
  const loss = Math.min(Math.floor((Math.random() * 9000 + 3500) * (user.premium ? 0.85 : 1)), Math.floor((user.coin || 0) * 0.55));
