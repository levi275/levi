    ...
    try {
        ...
        if (!character) {
            ...
            for (const c of characters) {
                ...
                if (nameNorm.includes(characterName)) {
                    best = c;
                    bestScore = 1;
                    break
                }
                ...
                for (const t of qTokens)
                    if (nameTokens.includes(normalize(t))) common++
                ...
                if (c.aliases && Array.isArray(c.aliases)) {
                    for (const a of c.aliases) {
                        ...
                        let commonA = 0
                        for (const t of qTokens)
                            if (aTokens.includes(normalize(t))) commonA++
        ...
        if (!character) {
            await conn.reply(m.chat, `《✧》No se ha encontrado el personaje *${characterNameRaw}*. Asegúrate de que el nombre esté correcto.`, m);
            return
        }
        ...
        // --- MODIFICACIÓN: FORZAR FORMATO IMAGEN ---
        // Si la URL termina en .webp, usamos una API externa para convertirla a PNG
        // Esto evita que WhatsApp la mande como sticker.
...
❖ Fuente » *${character.source}*`
        ...
        // Usamos sendFile. Al haber convertido la URL arriba, llegará como imagen.
