...
this.pushMessage(chatUpdate.messages).catch(e => console.error(e))
...
let participants = []
if (m.isGroup) {
groupMetadata = (this.chats[m.chat]?.metadata || await this.groupMetadata(m.chat).catch(e => null)) || {}
...
if (jid.endsWith('@lid')) { const pInfo = participants.find(p => p.lid === jid); return (pInfo && pInfo.id) ? pInfo.id : jid }
return jid
})
